...
test("explicitly imports project-only ChatGPT history with automatic history disabled", async ({ request }, testInfo) => {
  ...
  try {
    ...
    try {
      ...
      await expect(optionsPage.locator("#auto-sync-history")).not.toBeChecked();
      ...
      await expect.poll(() => serviceWorker.evaluate(async () => {
        const state = await chrome.storage.local.get("savemycontext.status");
        return Boolean((state["savemycontext.status"] as { historySyncInProgress?: boolean } | undefined)?.historySyncInProgress);
      })).toBe(false);
