...
async def journal_view(db: AsyncSession = Depends(get_db_session)) -> list[SessionListItem]:
...
async def factual_view(db: AsyncSession = Depends(get_db_session)) -> list[SessionListItem]:
    result = await db.execute(
        select(ChatSession)
...
async def ideas_view(db: AsyncSession = Depends(get_db_session)) -> list[SessionListItem]:
    result = await db.execute(
        select(ChatSession)
